...
cp ./apps/web-nextjs/.env.example ./apps/web-nextjs/.env
cp ./apps/scraper/.env.example ./apps/scraper/.env
```

For `web-nextjs`, also set Google OAuth and auth secrets (see `.env.example` for required vars):
...
### 2. Deploy `web-nextjs` to Vercel

You can deploy the `web-nextjs` frontend (Next.js) to [Vercel](https://vercel.com/).
...
2. In the Vercel project settings, add the following environment variables:
   ...
   - `AUTH_ALLOW_EMAILS`
   - `AUTH_SECRET`
3. Set the `Root Directory` in Vercel to `apps/web-nextjs`.
